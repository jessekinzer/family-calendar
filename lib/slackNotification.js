...
export async function sendSlackNotification(eventData, result) {
  ...
  const slackUserId = process.env.SLACK_USER_ID; // Optional: User ID for @mentions
  ...
  try {
    ...
    // Build mention text (use User ID if available, otherwise display name)
    const mentionText = slackUserId ? `<@${slackUserId}>` : '@Jesse';

    // Build the Slack message with blocks for rich formatting
    const slackMessage = {
      text: `${mentionText} 📅 New Calendar Event: ${eventData.title}`, // Fallback text with mention
      blocks: [
        {
          type: 'section',
          text: {
