...
/**
 * Fetch the stored refresh token from environment configuration.
...
export async function createCalendarEvent({ title, date, startTime, endTime, notes, isAllDay }) {
  ...
  const event = isAllDay
    ? buildAllDayEvent(title, date, notes, timeZone)
    : buildTimedEvent(title, date, startTime, endTime, notes, timeZone);
